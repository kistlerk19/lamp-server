...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $query = "UPDATE tasks SET title = ?, description = ?, priority = ?, due_date = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?";
            $stmt = $conn->prepare($query);
            
            // Handle empty due date
            $due_date_value = empty($due_date) ? null : $due_date;
            
            $stmt->bind_param("sssssi", $title, $description, $priority, $due_date_value, $status, $task_id);
            
            if ($stmt->execute()) {
                redirect_with_message('index.php', 'Task updated successfully!', 'success');
            } else {
                $errors[] = "Error updating task. Please try again.";
            }
    ...
    $due_date = $task['due_date'] === '0000-00-00' ? '' : $task['due_date'];
    $status = $task['status'];
}
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
        <div class="container">
            ...
            <div class="navbar-nav ms-auto">
                ...
                <a class="nav-link" href="view.php?id=<?php echo $task_id; ?>">
            ...
            <div class="col-lg-8">
                <div class="card fade-in-up">
                    <div class="card-header">
                        ...
                        <p class="mb-0">Modify the task details below</p>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($errors)): ?>
                        ...
                        <div class="alert alert-info">
                            ...
                            <p class="mb-1"><strong>Created:</strong> <?php echo format_date($task['created_at'], 'M d, Y \a\t g:i A'); ?></p>
                            <?php if ($task['updated_at'] !== $task['created_at']): ?>
                                <p class="mb-0"><strong>Last Updated:</strong> <?php echo format_date($task['updated_at'], 'M d, Y \a\t g:i A'); ?></p>
                            <?php endif; ?>
                        </div>
                        ...
                        <form method="POST" class="needs-validation" novalidate>
                            ...
                            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                ...
                                <a href="view.php?id=<?php echo $task_id; ?>" class="btn btn-info me-md-2">
                                    <i class="bi bi-eye"></i> View Task
                                </a>
